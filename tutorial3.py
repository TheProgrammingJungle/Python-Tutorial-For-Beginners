...
# This print statement will throw an error as "input" function always gives(or
# returns) a string, and int and str operands are not compatible for
# subtraction.
# print("I am left with " + (200 - dollars_taken) + " dollars")

# This print statement will throw a different error, that int and string are
# not compatible for concatenation, as now the middle operand is an int,
# whereas other two are strings. 
# print("I am left with " + (200 - int(dollars_taken) + " dollars")

# This print will work fine, as first dollars_taken is converted to int, so
# (200 - int(dollars_taken)) is still an int. Post that we convert it to
# string, which can now be concatenated with other strings.
...
# int("i") -> Will throw an error, as values passed to this type conversion
# function should be compatible.
# int("34") -> Works fine.

# Special scenario for bool:
# i) Any non-zero number is converted to True, and any zero number(0, 0.0, etc)
# is converted to False.
# ii) Any non-empty string is converted to True, and any empty string ("", '')
# is converted to False.
...
# This will throw an error, as "two" is a string and does not correspond
# directly to int format.
# a = int("two")
b = int(4.5) 	# 4
c = int("1029") # 1029
d = int(False)  # 0
...
l = str(78.948)  # '78.948'
m = str(True)    # 'True'
# Here false is not a string, nor a boolean value(True, False, case sensitive).
# Hence false is not known. 
# n = str(false)
...
q = bool(1)       # True
r = bool(0.0)     # False
# Here string is 'False', but still boolean value will be True as this is non
# empty string.
s = bool("False") # True
